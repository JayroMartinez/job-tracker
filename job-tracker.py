...
def render_row_actions(idx, row):
    ...
    # Reject
    key_reject = f"reject_{row.id}_{idx}"
    if not row.rejected:
        if c1.button("Reject", key=key_reject):
    ...
    # Edit
    key_edit = f"edit_{row.id}_{idx}"
    if c2.button("Edit", key=key_edit):
        st.session_state["edit_id"] = row.id
        st.session_state["show_form"] = False
        st.rerun()

    # Delete + confirm
    key_del     = f"delete_{row.id}_{idx}"
    key_conf    = f"confirm_{row.id}_{idx}"
    key_cancel  = f"cancel_{row.id}_{idx}"
    cid = st.session_state.get("confirm_delete_id")
    if cid == row.id:
        c3.markdown("<span style='color:red'>Confirm</span>", unsafe_allow_html=True)
        if c3.button("Confirm", key=key_conf):
            df2 = st.session_state["df"].drop(idx).reset_index(drop=True)
            st.session_state["df"]()_
